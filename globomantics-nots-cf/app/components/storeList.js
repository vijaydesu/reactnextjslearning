...
import StoreFuelPriceRowMem from "./storeFuelPriceRow";
...
/*   const [fuelProducts, setFuelProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchStoreFuelPrices = async (storeNo) => {
      ...
      <table className="table-fixed w-full bg-white border border-gray-300 ">
        ...
        <tbody>
          {fuelProducts?.map((fp) => (
            ...
            <StoreFuelPriceRowMem key={fp.productCode} fuelProduct={fp} />
